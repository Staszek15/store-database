...
author: "Wiktoria Fimińska 262283, Julia Grzegorzewska 262314, Karolina Wypych, Mateusz Stasiak"
date: "`r Sys.Date()`"
fontsize: 12pt
output: 
  pdf_document:
...
```{python}
query1_1 = connection.execute(text('''WITH tab AS (
                            ...
                            ORDER BY COUNT(*) DESC
                            LIMIT 1'''))
df1_1 = pd.DataFrame(query1_1.fetchall(), columns=query1_1.keys())
best_employee = df1_1['employee'][0]
how_often = df1_1['how_often'][0]
```

Pracownikiem, który został w tym okresie najczęściej wyróżniony jest (KTO - best_employee), który tytuł ten otrzymał (ILE - how_often) razy. 
...
Najbardziej dochodowa gra z wypożyczeń (tabela \ref{tab:tab3})
